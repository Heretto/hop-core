...
- **Python 3.11+.**
- **Angular 22 and Node ≥ 22.22.3 or ≥ 24.15** for the frontend.
  `@heretto/hop-ui` peers on `^22`, so an app on an older Angular fails to
  install it, and the Angular 22 CLI refuses older Node.
- **Keep Zone.js change detection.** `ng update` to 21+ inserts
  `provideZoneChangeDetection()` into `bootstrapApplication` — leave it. The
  hop-ui components are not zoneless-safe: in a zoneless app, state they set in
  HTTP callbacks never renders (a failed login leaves the spinner running and
  shows no error). New apps generated by the Angular 21+ CLI are zoneless by
  default, so add the provider and the `zone.js` polyfill back.
